...
@section('page-content')
    ...
    <form action="{{ route('principal.class-schedule.post') }}" method="post" id='form_sections'>
        @csrf
                                ...
                                <td class="p-1">
                                    @foreach ($section->allocations()->havingLectureNo($lecture->lecture_no)->get() as $allocation)
                                        <div class="text-sm bg-teal-50">
                                            <a href="{{ route('principal.section.lecture.schedule.edit', [$section, $lecture->lecture_no, $allocation]) }}"
                                                class="link">{{ $allocation->subject->short_name }}</a>
                                            <p>{{ $allocation->user->short_name }}</p>
                                        </div>
                                        <div class="divider"></div>
                                    @endforeach
                                    <a href="{{ route('principal.section.lecture.schedule.create', [$section, $lecture->lecture_no]) }}"
