...
@section('page-content')
    ...
    <div class="flex flex-wrap justify-between gap-3 items-center mt-8">
        ...
        <div class="flex items-center gap-3">
            ...
            <a href="{{ route('user.test-allocation.import-students.index', $testAllocation) }}"
            ...
            @if ($testAllocation->appearingStudents->count())
                <a href="{{ route('user.test-allocation.results.edit', [$testAllocation, 0]) }}"
            ...
            <tbody>
                @foreach ($testAllocation->results->sortBy('student.rollno') as $result)
                    <tr class="tr">
                        ...
                        <td>
                            <form action="{{ route('user.test-allocation.results.destroy', [$testAllocation, $result]) }}"
    ...
    @if ($testAllocation->appearingStudents->count())
        <div class="md:w-2/3 mx-auto mt-8 text-center">
            ...
            <form action="{{ route('user.test.test-allocations.update', [$testAllocation->test, $testAllocation]) }}"
