...
@section('page-content')
    ...
    <div class="bread-crumb">
        ...
        <div>/</div>
        <a href="{{ route('user.test-allocation.results.index', $testAllocation) }}">Result</a>
        <div>/</div>
        <div>Edit</div>
    </div>
    ...
    <form action="{{ route('user.test-allocation.results.update', [$testAllocation, 1]) }}" method="post"
        onsubmit="return validate(event)">
        @csrf
        @method('patch')
        <div class="flex flex-wrap items-center gap-x-16 gap-y-3 mt-8">
            <h2 class="text-teal-600">{{ $testAllocation->subject->name }}, {{ $testAllocation->section->name }}</h2>
