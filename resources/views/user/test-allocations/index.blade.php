...
@section('page-content')
    ...
    <div class="content-section">
        ...
        @if (Auth::user()->user?->isIncharge())
            @php $section = Auth::user()->user->sectionAsIncharge(); @endphp
            <h2 class="mt-8 flex items-center"><i class="bi-diagram-3 mr-2"></i> My Class: {{ $section->name }}
            </h2>
                ...
                <tbody>
                    @foreach ($testAllocations as $testAllocation)
                        <tr class="tr @if ($testAllocation->hasBeenSubmitted()) submitted @endif">
                            <td>{{ $loop->index + 1 }}</td>
                            <td class="text-left">
                                <a href="{{ route('user.test-allocation.results.index', $testAllocation) }}"
                                    class="flex items-center link">
                                    {{ $testAllocation->subject->name }} - {{ $testAllocation->section->name }}
