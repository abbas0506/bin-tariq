...
class AttendanceController extends Controller
{
    ...
    public function index($id)
    {
        ...
        $present = $section->students()
            ->whereHas('attendances', function ($q) {
                $q->whereDate('attendances.date', today())
                    ->where('attendances.status', 1);
            })
            ->where('students.status', 1) // active students
            ->count();
        // $present = $section->attendances()->whereDate('date', today())->where('status', 1)->count();
        return view('user.section-attendance.index', compact('section', 'present'));
    }
    ...
    public function create($id)
    {
        //
        $section = Section::findOrFail($id);
        return view('user.section-attendance.create', compact('section'));
    }
    ...
    public function store(Request $request, $id)
    {
        ...
        try {
            ...
            DB::commit();
            return redirect()->route('user.section.attendance.index', $section);
        } catch (Exception $ex) {
            Db::rollBack();
    ...
    public function show($id)
    {
        ...
        return view('user.section-attendance.show', compact('section', 'attendances'));
    }
    ...
    public function edit($id)
    {
        ...
        $attendances = $section->attendances()->whereDate('date', today())->get();
        return view('user.section-attendance.edit', compact('section', 'attendances'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            DB::commit();
            return redirect()->route('user.section.attendance.index', $section)->with('success', 'Attendance successfully updated');
        } catch (Exception $e) {
            DB::rollBack();
